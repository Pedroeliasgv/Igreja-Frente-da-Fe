...
export const Route = createFileRoute("/online")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:image", content: praiseImg },
      { name: "twitter:image", content: praiseImg },
    ],
  }),
});
    ...
    <SiteLayout>
      <div ref={ref}>
        <section className="relative isolate overflow-hidden bg-ink pb-20 pt-40 lg:pb-28 lg:pt-48">
          <img src={praiseImg} alt="" aria-hidden="true" className="absolute inset-0 h-full w-full object-cover opacity-30" loading="eager" width={1280} height={896} />
        ...
        <section className="bg-background py-20 lg:py-24">
          <div className="mx-auto max-w-6xl px-6 lg:px-10">
            <div className="reveal aspect-video overflow-hidden rounded-3xl bg-ink shadow-elegant">
              <iframe
            ...
            <div className="mt-8 flex flex-wrap items-center justify-between gap-4">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-3 rounded-full bg-red-600 px-6 py-3 text-sm font-semibold text-white shadow-soft transition-transform hover:scale-105"
              >
        ...
        <section className="bg-secondary/40 py-24 lg:py-32">
          <div className="mx-auto max-w-7xl px-6 lg:px-10">
            <SectionHeader
              ...
              description="Reveja mensagens, alimente sua fé e compartilhe com quem precisa."
            />
            <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {recent.map((v, i) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="reveal group block overflow-hidden rounded-2xl bg-card border border-border hover-lift"
                ...
                >
                  <div className="relative aspect-video overflow-hidden bg-ink">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-gold text-ink shadow-gold transition-transform group-hover:scale-110">
                        <Play className="h-6 w-6 fill-current" />
                      </span>
                    </div>
                    <span className="absolute bottom-3 right-3 rounded-md bg-ink/80 px-2 py-1 text-xs text-ivory">
                      {v.duration}
                    </span>
                  </div>
                  <div className="p-6">
                    <h3 className="font-serif text-xl text-foreground">{v.title}</h3>
                    <p className="mt-1 text-sm text-muted-foreground">{v.pastor}</p>
                  </div>
                </a>
              ))}
            </div>
